...
public abstract class MainStriker extends FrameLayout{

    public static final String TAG = "com.knef.MainStriker";
    ...
    public MainStriker(Context context) {
        super(context);
        init(context);
    }

    public MainStriker(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public MainStriker(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init(Context context){
        ...
        this.iv_delete.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View view) {
                if(MainStriker.this.getParent()!=null){
                    ViewGroup myCanvas = ((ViewGroup)MainStriker.this.getParent());
                    myCanvas.removeView(MainStriker.this);
                }
            }
        });
            ...
            if(view.getTag().equals("DraggableViewGroup")) {
                switch (event.getAction()) {
                    ...
                    case MotionEvent.ACTION_MOVE:
                        ...
                        MainStriker.this.setX(MainStriker.this.getX()+offsetX);
                        MainStriker.this.setY(MainStriker.this.getY() + offsetY);
                        move_orgX = event.getRawX();
                        move_orgY = event.getRawY();
                        break;
                ...
                switch (event.getAction()) {
                    case MotionEvent.ACTION_DOWN:
                        ...
                        this_orgX = MainStriker.this.getX();
                        this_orgY = MainStriker.this.getY();

                        scale_orgX = event.getRawX();
                        scale_orgY = event.getRawY();
                        scale_orgWidth = MainStriker.this.getLayoutParams().width;
                        scale_orgHeight = MainStriker.this.getLayoutParams().height;

                        rotate_orgX = event.getRawX();
                        rotate_orgY = event.getRawY();

                        centerX = MainStriker.this.getX()+
                                ((View)MainStriker.this.getParent()).getX()+
                                (float)MainStriker.this.getWidth()/2;
                        ...
                        centerY = MainStriker.this.getY()+
                                ((View)MainStriker.this.getParent()).getY()+
                                statusBarHeight+
                                (float)MainStriker.this.getHeight()/2;
                    ...
                    case MotionEvent.ACTION_MOVE:
                        ...
                        if(length2 > length1
                            ...
                            MainStriker.this.getLayoutParams().width += offset;
                            MainStriker.this.getLayoutParams().height += offset;
                            onScaling(true);
                            //DraggableViewGroup.this.setX((float) (getX() - offset / 2));
                            //DraggableViewGroup.this.setY((float) (getY() - offset / 2));
                        }else if(length2 < length1
                                && (angle_diff < 25 || Math.abs(angle_diff-180)<25)
                                && MainStriker.this.getLayoutParams().width > size/2
                                && MainStriker.this.getLayoutParams().height > size/2) {
                            //scale down
                            double offsetX = Math.abs(event.getRawX() - scale_orgX);
                            double offsetY = Math.abs(event.getRawY() - scale_orgY);
                            double offset = Math.max(offsetX, offsetY);
